...
use hj_core::{
    Handoff, HandoffItem, HandoffState, HandupItem, HandupProject, HandupRecommendation,
    HandupReport, LogEntry,
};
use hj_doob::{
    DoobClient, ReconcileReport, TodoStatus, ensure_doob_on_path, map_priority, unique_titles,
};
use hj_git::{
    RepoContext, SurveyHandoff, TodoMarker, branch_name, current_short_head, discover,
    discover_handoffs, discover_todo_markers, today,
};
...
fn handup(args: HandupArgs) -> Result<()> {
    let cwd =
